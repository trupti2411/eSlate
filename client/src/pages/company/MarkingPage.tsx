...
import {
  ArrowLeft, ArrowRight, CheckCircle, FileText, Eye,
...
export function MarkingPage() {
  ...
  const handleQuickScore = (s: number) => {
    setScore(s);
  };
    ...
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <div className="bg-teal-600 text-white px-4 py-3 flex items-center gap-3 flex-shrink-0">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-teal-200">{pending.length} remaining</p>
        </div>
        <span className="text-xs font-bold bg-white/20 rounded-lg px-2.5 py-1">
          {currentIndex + 1} / {pending.length || 1}
        </span>
      </div>

      {/* Progress strip */}
      {allSubmissions.length > 0 && (
        <div className="bg-teal-700 px-4 pb-2">
          ...
          <p className="text-xs text-teal-300 mt-1">{gradedIds.size} of {allSubmissions.length} marked this session</p>
        </div>
      )}
      ...
      {pending.length === 0 ? (
        ...
        <>
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
                ...
                <div className="grid grid-cols-4 gap-2 mb-4">
                  {QUICK_SCORES.map(q => (
                    <button
                      key={q.label}
                      onClick={() => handleQuickScore(q.score)}
          ...
          {/* Bottom action bar */}
          <div className="sticky bottom-0 bg-white border-t border-gray-100 px-4 py-3 flex gap-3 flex-shrink-0">
            <button
