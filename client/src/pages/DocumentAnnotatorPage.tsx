...
export default function DocumentAnnotatorPage() {
  ...
  const loadDocument = useCallback(async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `${documentType} is ready for annotation. Use the tools on the left to mark up the document.`,
      });
  ...
  }, [documentUrl, documentType, toast]);
  ...
  const initializeCanvas = useCallback(() => {
    if (canvasRef.current && containerRef.current) {
      ...
      // Set canvas to cover the entire container
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500 mt-2">Preparing annotation interface...</p>
        </div>
      </div>
    );
  }
