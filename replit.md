...
The Company Admin dashboard includes an "Auto-Setup Australia 2026" feature in the Academic Management section. This auto-creates 4 terms (Term 1–4) and their weekly breakdowns for a selected year level, using official government term dates for any Australian state/territory. The state is auto-detected from the company's profile (set via Settings → Business Location), or can be manually overridden in the dialog. All 8 states/territories are supported: NSW (Eastern & Western division), VIC, QLD, SA, WA, TAS, ACT, NT. The `academic_weeks` table stores week numbers, names, and start/end dates per term. Weeks are displayed on demand by clicking "Show weeks" on any term card. The `tutoringCompanies` table has a `state` field (varchar 10) storing the AU state code.
